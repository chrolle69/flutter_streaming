...
class AddStreamPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      ...
      child: AddStreamPageHelper(onStreamAdded: onStreamAdded,),
    );
  }
}
      ...
      floatingActionButton: Row(
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            onPressed: () async {
              var dataState = await Provider.of<AddStreamState>(context, listen: false).addStream();
              print("going back");
              if (dataState is DataSuccess){
                if (dataState.data is StreamModel) {
                  var stream = dataState.data as StreamModel;
                  Navigator.of(context).push(MaterialPageRoute(builder: (context) => RoomPage(roomId: stream.id, onFetchStreams: onStreamAdded)));
                }
              }
            },
          )
